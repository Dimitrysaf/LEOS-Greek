...
public class MavenXmlCopyrightsMapping extends BaseXmlLoader {
    ...
    public MavenXmlCopyrightsMapping(Path copyrightsXml) throws ParserConfigurationException, IOException, SAXException {
    ...
    private void populateMaps(XmlArtifact xmlArtifact) {
        ...
        xmlArtifact.getArtifactIds()
                ...
                .forEach(groupIdAndArtifactId -> {
                    ...
                    String groupIdAndArtifactIdAndVersion = String.format("%s:%s", groupIdAndArtifactId,
    ...
    private List<String> getLicenseAliases(Element xmlArtifact) {
        Element licenseEl =  getChildElement(xmlArtifact, "license");
        List<Element> licenses = getChildElements(licenseEl, "name");
        return licenses.stream()
                .map(this::findChildText)
                .collect(Collectors.toList());
    }
