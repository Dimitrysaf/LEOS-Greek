...
------ source repository: 'leos_dev_cn' (see in leos_cmis.repository all possible repo sources)
------ target schema: leos_repository_cn. (see all users created on oracle database, leos_repository for EC, leos_repository_cn for CN)
...
--1. Packages
INSERT INTO leos_repository_cn.package (OBJECT_ID,NAME,AUDIT_C_BY,AUDIT_C_DATE, audit_last_m_by, audit_last_m_date)
select
    object_id, cmis_name,
    cmis_createdby,
...
from (
     ...
     ) where cmis_basetypeid = 'cmis:folder' and repository_name = 'leos_dev_cn'
         and parent_id in (select object_id from object_path_view where object_path = '/leos/workspaces' and repository_id = 'leos_dev_cn')
;

--1. generate document categories
INSERT INTO leos_repository_cn.document_categories (category_code, category_desc, audit_c_by, audit_c_date)
...
--2. Document
INSERT INTO leos_repository_cn.document (PACKAGE_ID,CATEGORY_ID,NAME,REVISION_STATUS,CONTRIBUTION_STATUS,ORIGIN_REF,
...
from (
         ...
         from (
                  ...
                  from leos_cmis lc, object_path_view opv
                  where lc.object_id = opv.object_id
              ) src, (select id, object_id from leos_repository_cn.package) ora_cn_pkg, leos_repository_cn.document_categories doccat
         where src.cmis_basetypeid = 'cmis:document' and src.repository_name = 'leos_dev_cn'
...
INSERT INTO leos_repository_cn.document (PACKAGE_ID,CATEGORY_ID,is_archived,NAME,REVISION_STATUS,CONTRIBUTION_STATUS,ORIGIN_REF,
                                         BASE_REVISION_ID,LIVE_DIFFING_REQUIRED,REF,PROCEDURE_TYPE,DOC_TEMPLATE,LANGUAGE,DOC_STAGE,IS_PRIVATE_WORKING_COPY,
                                         AUDIT_C_BY,AUDIT_C_DATE,AUDIT_LAST_M_BY,AUDIT_LAST_M_DATE)
...
from (
         ...
         from (
                  ...
                  from leos_cmis lc, object_path_view opv
                  where lc.object_id = opv.object_id
                    and opv.object_path like '%archive%' and opv.type_id = 'cmis:document' and opv.repository_id = 'leos_dev_cn'
              ) src, (select id, object_id from leos_repository_cn.package) ora_cn_pkg, leos_repository_cn.document_categories doccat,
...
INSERT INTO leos_repository_cn.document_VERSION
(object_id,DOCUMENT_ID,VERSION_LABEL,VERSION_SERIES_ID,VERSION_TYPE,IS_LATEST_MAJOR_VERSION,IS_LATEST_VERSION,IS_MAJOR_VERSION,
 IS_VERSION_SERIES_CHECKED_OUT,IS_IMMUTABLE,COMMENTS,
 AUDIT_C_BY,AUDIT_C_DATE,AUDIT_LAST_M_BY,AUDIT_LAST_M_DATE)
...
from (
         ...
         from leos_cmis lc, object_path_view opv
         where lc.object_id = opv.object_id
     ) src, (select id, name from leos_repository_cn.document
--where is_archived = 1
     ) ora_cn_doc
where src.cmis_basetypeid = 'cmis:document' and src.repository_name = 'leos_dev_cn'
  and src.cmis_name = ora_cn_doc.name
order by ora_cn_doc.id, src.object_id
;

--3. document_content for all docs and archived also
INSERT INTO leos_repository_cn.DOCUMENT_CONTENT
...
from (
         ...
         from leos_cmis lc, object_path_view opv
         where lc.object_id = opv.object_id
     ) src, (select id, document_id, object_id from leos_repository_cn.document_version
--where document_id in (select id from leos_repository_cn.document where is_archived =1)
     ) ora_ver, STREAM str
where src.cmis_basetypeid = 'cmis:document' and src.repository_name = 'leos_dev_cn'
  and src.object_id = ora_ver.object_id
  and leos_Template is not null
  and src.object_id = str.id
order by ora_ver.document_id, src.object_id
;

--4. DOCUMENT_MILESTONE
INSERT INTO leos_repository_cn.DOCUMENT_MILESTONE
(DOCUMENT_ID,CATEGORY_CODE,JOB_ID,JOB_DATE,EXPORT_STATUS,EXPORT_DATE,CONTENT,STATUS,MILESTONE_COMMENTS,AUDIT_C_BY,AUDIT_C_DATE,AUDIT_LAST_M_BY,AUDIT_LAST_M_DATE)
SELECT
    ora_ver.id document_id,
    LEOS_CATEGORY,
    leos_jobId, to_timestamp(cmis_creationDate,'YYYY-MM-DD"T"HH24:MI:SS.FF3"Z"') job_date,
    leos_revisionStatus,
    to_timestamp(leos_initialCreationDate,'YYYY-MM-DD"T"HH24:MI:SS.FF3"Z"') leos_initialCreationDate,
    str.data,
    leos_status, leos_comments,
    cmis_createdby, to_timestamp(cmis_creationdate,'YYYY-MM-DD"T"HH24:MI:SS.FF3"Z"') audit_c_date,
    cmis_lastmodifiedby, to_timestamp(cmis_lastmodificationdate,'YYYY-MM-DD"T"HH24:MI:SS.FF3"Z"') audit_last_m_date
from (
         select opv.parent_id, opv.repository_id repository_name, lc.*
         from leos_cmis lc, object_path_view opv
         where lc.object_id = opv.object_id
     ) src, (select id, name from leos_repository_cn.document) ora_ver, STREAM str
where src.cmis_basetypeid = 'cmis:document' and src.repository_name = 'leos_dev_cn'
  and src.cmis_name = ora_ver.name
  and metadata_docTemplate is null
  and src.object_id = str.id
order by ora_ver.id, src.object_id;

--5. DOCUMENT_MILESTONE_LIST
INSERT INTO leos_repository_cn.DOCUMENT_MILESTONE_LIST(MILESTONE_ID,CONTAINED_DOCUMENTS,AUDIT_C_BY,AUDIT_C_DATE,AUDIT_LAST_M_BY,AUDIT_LAST_M_DATE)
...
from (
         ...
         where repository_id = (select id from repository where cmis_id = 'leos_dev_cn')
         group by repository_id, object_id) src, (SELECT MILESTONE_ID, NAME FROM leos_repository_cn.MILESTONE_V) ora
...
INSERT INTO CONFIG_CATEGORIES (CATEGORY_CODE, CATEGORY_DESC, AUDIT_C_BY, AUDIT_C_DATE) VALUES ('CONFIG','Configuration', 'LEOS_CMIS', to_timestamp('06-APR-23 12:26:17.000000000','DD-MON-RR HH24:MI:SSXFF'));
INSERT INTO CONFIG_CATEGORIES (CATEGORY_CODE, CATEGORY_DESC, AUDIT_C_BY, AUDIT_C_DATE) VALUES ('STRUCTURE','Structure', 'LEOS_CMIS', to_timestamp('06-APR-23 12:26:17.000000000','DD-MON-RR HH24:MI:SSXFF'));
INSERT INTO CONFIG_CATEGORIES (CATEGORY_CODE, CATEGORY_DESC, AUDIT_C_BY, AUDIT_C_DATE) VALUES ('TEMPLATE_PROPOSAL','Template', 'LEOS_CMIS', to_timestamp('06-APR-23 12:26:17.000000000','DD-MON-RR HH24:MI:SSXFF'));
INSERT INTO CONFIG_CATEGORIES (CATEGORY_CODE, CATEGORY_DESC, AUDIT_C_BY, AUDIT_C_DATE) VALUES ('TEMPLATE_BILL','Template', 'LEOS_CMIS', to_timestamp('06-APR-23 12:26:17.000000000','DD-MON-RR HH24:MI:SSXFF'));
INSERT INTO CONFIG_CATEGORIES (CATEGORY_CODE, CATEGORY_DESC, AUDIT_C_BY, AUDIT_C_DATE) VALUES ('TEMPLATE_MEMORANDUM','Template', 'LEOS_CMIS', to_timestamp('06-APR-23 12:26:17.000000000','DD-MON-RR HH24:MI:SSXFF'));
INSERT INTO CONFIG_CATEGORIES (CATEGORY_CODE, CATEGORY_DESC, AUDIT_C_BY, AUDIT_C_DATE) VALUES ('TEMPLATE_ANNEX','Template', 'LEOS_CMIS', to_timestamp('06-APR-23 12:26:17.000000000','DD-MON-RR HH24:MI:SSXFF'));
INSERT INTO CONFIG_CATEGORIES (CATEGORY_CODE, CATEGORY_DESC, AUDIT_C_BY, AUDIT_C_DATE) VALUES ('TEMPLATE_COUNCIL_EXPLANATORY','Template', 'LEOS_CMIS', to_timestamp('06-APR-23 12:26:17.000000000','DD-MON-RR HH24:MI:SSXFF'));
INSERT INTO CONFIG_CATEGORIES (CATEGORY_CODE, CATEGORY_DESC, AUDIT_C_BY, AUDIT_C_DATE) VALUES ('TEMPLATE_STAT_FINANC_LEGIS','Template', 'LEOS_CMIS', to_timestamp('06-APR-23 12:26:17.000000000','DD-MON-RR HH24:MI:SSXFF'));
INSERT INTO CONFIG_CATEGORIES (CATEGORY_CODE, CATEGORY_DESC, AUDIT_C_BY, AUDIT_C_DATE) VALUES ('TEMPLATE','Template', 'LEOS_CMIS', to_timestamp('06-APR-23 12:26:17.000000000','DD-MON-RR HH24:MI:SSXFF'));

--6. CONFIG
INSERT INTO leos_repository_CN.CONFIG (NAME,OBJECT_ID,CATEGORY_ID,LANGUAGE,AUDIT_C_BY,AUDIT_C_DATE,AUDIT_LAST_M_BY,AUDIT_LAST_M_DATE)
select
...
from (
         ...
         from (
              ...
              leos_repository_CN.config_categories confcat
         where src.cmis_basetypeid = 'cmis:document' and src.repository_name = 'leos_dev_cn'
           and src.parent_id = (SELECT object_id FROM object_path_view WHERE OBJECT_PATH = '/leos/templates' and repository_id = 'leos_dev_cn')
...
--7. CONFIG_VERSION
INSERT INTO leos_repository_cn.config_version
...
from (
         ...
         from leos_cmis lc, object_path_view opv
         where lc.object_id = opv.object_id
     ) src, leos_repository_CN.config cfg
where src.cmis_basetypeid = 'cmis:document' and src.repository_name = 'leos_dev_cn'
  and src.parent_id = (SELECT object_id FROM object_path_view WHERE OBJECT_PATH = '/leos/templates' and repository_id = 'leos_dev_cn')
  and src.object_id = cfg.object_id;

--8. CONFIG_CONTENT
INSERT INTO leos_repository_cn.CONFIG_CONTENT (VERSION_ID,CONTENT,
...
from (
         ...
         from leos_cmis lc, object_path_view opv
         where lc.object_id = opv.object_id
     ) src, leos_repository_CN.config_version cfg, STREAM str
where src.cmis_basetypeid = 'cmis:document' and src.repository_name = 'leos_dev_cn'
  and src.parent_id = (SELECT object_id FROM object_path_view WHERE OBJECT_PATH = '/leos/templates' and repository_id = 'leos_dev_cn')
  and src.object_id = str.id
  and src.object_id = cfg.object_id;
...
--9. COLLABORATORS
INSERT INTO LEOS_REPOSITORY_CN.COLLABORATORS(COLLABORATOR_NAME,ROLE_ID,ORGANIZATION,AUDIT_C_BY,AUDIT_C_DATE)
...
INSERT INTO LEOS_REPOSITORY_CN.PACKAGE_COLLABORATORS (PACKAGE_ID,COLLABORATOR_ID,AUDIT_C_BY,AUDIT_C_DATE)
select PACKAGE_ID, ID, AUDIT_C_BY,AUDIT_C_DATE from (
                                                        select
                                                            distinct package_id, leos_collaborators
                                                        FROM LEOS_CMIS lc, leos_repository_cn.document_version docver, leos_repository_cn.document doc
                                                        WHERE leos_collaborators is not null and repository_id in (select id from repository where cmis_id = 'leos_dev_cn')
                                                          AND lc.object_id = docver.object_id
                                                          and docver.document_id = doc.id) src, leos_repository_cn.collaborators colb
WHERE src.leos_collaborators = colb.collaborator_name ||'::'||colb.role_id||'::'||colb.organization;

--11. DOCUMENT PROPERTIES FLAGS
INSERT INTO leos_repository_cn.DOCUMENT_PROPERTIES (PROPERTY_NAME,DOC_CATEGORY_ID,AUDIT_C_BY,AUDIT_C_DATE,AUDIT_LAST_M_BY,AUDIT_LAST_M_DATE) VALUES ('clonedRef',(SELECT id FROM leos_repository_cn.DOCUMENT_CATEGORIES WHERE CATEGORY_CODE='ANNEX'),'LEOS_CMIS',to_timestamp('14-APR-23 10:00:05.000000000','DD-MON-RR HH24:MI:SSXFF'),null,null);
INSERT INTO leos_repository_cn.DOCUMENT_PROPERTIES (PROPERTY_NAME,DOC_CATEGORY_ID,AUDIT_C_BY,AUDIT_C_DATE,AUDIT_LAST_M_BY,AUDIT_LAST_M_DATE) VALUES ('docIndex',(SELECT id FROM leos_repository_cn.DOCUMENT_CATEGORIES WHERE CATEGORY_CODE='ANNEX'),'LEOS_CMIS',to_timestamp('14-APR-23 10:00:05.000000000','DD-MON-RR HH24:MI:SSXFF'),null,null);
INSERT INTO leos_repository_cn.DOCUMENT_PROPERTIES (PROPERTY_NAME,DOC_CATEGORY_ID,AUDIT_C_BY,AUDIT_C_DATE,AUDIT_LAST_M_BY,AUDIT_LAST_M_DATE) VALUES ('docNumber',(SELECT id FROM leos_repository_cn.DOCUMENT_CATEGORIES WHERE CATEGORY_CODE='ANNEX'),'LEOS_CMIS',to_timestamp('14-APR-23 10:00:05.000000000','DD-MON-RR HH24:MI:SSXFF'),null,null);
INSERT INTO leos_repository_cn.DOCUMENT_PROPERTIES (PROPERTY_NAME,DOC_CATEGORY_ID,AUDIT_C_BY,AUDIT_C_DATE,AUDIT_LAST_M_BY,AUDIT_LAST_M_DATE) VALUES ('docTitle',(SELECT id FROM leos_repository_cn.DOCUMENT_CATEGORIES WHERE CATEGORY_CODE='ANNEX'),'LEOS_CMIS',to_timestamp('14-APR-23 10:00:05.000000000','DD-MON-RR HH24:MI:SSXFF'),null,null);
INSERT INTO leos_repository_cn.document_properties (property_name, DOC_CATEGORY_ID, AUDIT_C_BY, AUDIT_C_DATE)
select property_name, id, 'LEOS_CMIS', to_timestamp('14-APR-23 10:00:05.000000000','DD-MON-RR HH24:MI:SSXFF') from (
...
from (
    ...
    from leos_cmis where annex_clonedref is not null and object_id in (select object_id from object_path_view where repository_id = 'leos_dev_cn')
) src, leos_repository_cn.document_version ver
   ,(select id property_id from leos_repository_cn.document_properties where property_name = 'clonedRef') docprop
...
from (
    ...
    from leos_cmis where annex_docIndex is not null and object_id in (select object_id from object_path_view where repository_id = 'leos_dev_cn')
) src, leos_repository_cn.document_version ver
   ,(select id property_id from leos_repository_cn.document_properties where property_name = 'docIndex') docprop
where src.object_id = ver.object_id;
...
from (
    ...
    from leos_cmis where annex_docNumber is not null and object_id in (select object_id from object_path_view where repository_id = 'leos_dev_cn')
) src, leos_repository_cn.document_version ver
   ,(select id property_id from leos_repository_cn.document_properties where property_name = 'docNumber') docprop
where src.object_id = ver.object_id;
...
from (
    ...
    from leos_cmis where annex_docTitle is not null and object_id in (select object_id from object_path_view where repository_id = 'leos_dev_cn')
) src, leos_repository_cn.document_version ver
   ,(select id property_id from leos_repository_cn.document_properties where property_name = 'docTitle') docprop
where src.object_id = ver.object_id;
...
from (
    ...
    from leos_cmis where leos_clonedMilestoneId is not null and object_id in (select object_id from object_path_view where repository_id = 'leos_dev_cn')
) src, leos_repository_cn.document_version ver
   ,(select id property_id from leos_repository_cn.document_properties where property_name = 'clonedMilestoneId') docprop
where src.object_id = ver.object_id;
...
from (
    ...
    from leos_cmis where leos_clonedProposal is not null and object_id in (select object_id from object_path_view where repository_id = 'leos_dev_cn')
) src, leos_repository_cn.document_version ver
