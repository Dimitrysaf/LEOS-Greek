...
public interface DocumentContentRepository extends JpaRepository<DocumentContent, BigDecimal> {
    ...
    @Query(value = "SELECT c.ORIGINAL_FILE_NAME as originalFilename, LENGTH(c.BINARY_CONTENT) as binaryContentSize, c.FOREIGN_RENDITION_ORIGINAL_FILE_NAME as foreignRenditionOriginalFilename FROM DOCUMENT_CONTENT c WHERE c.VERSION_ID = ?1", nativeQuery = true)
