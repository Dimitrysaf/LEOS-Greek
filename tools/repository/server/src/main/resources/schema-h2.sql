...
(	ID NUMBER(22,0) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     ...
     AUDIT_LAST_M_BY VARCHAR2(30 BYTE),
     AUDIT_LAST_M_DATE TIMESTAMP (6),
     CREATOR_ORGANIZATION VARCHAR2(100)
);
...
(	ID NUMBER(22,0) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
     ...
     FOREIGN_RENDITION_CONTENT BLOB,
     FOREIGN_RENDITION_ORIGINAL_FILE_NAME VARCHAR(255),
...
SELECT doc.id||'_'||docmil.id unique_id, doc.id document_id,doc.package_id,pkg.name package_name,doc.object_id doc_object_id,doc.category_id,doccat.category_code,doc.name,doc.cloned_from,doc.revision_status,doc.contribution_status,doc.origin_ref,doc.base_revision_id,doc.live_diffing_required,doc.ref,doc.procedure_type,
       ...
       docmil.status,docmil.audit_c_by,docmil.audit_c_date, docmil.audit_last_m_date, docmil.audit_last_m_by, doc.custom_template_act
...
ALTER TABLE CONFIG_VERSION
    ADD CONSTRAINT CONFIG_VERSION_UQ UNIQUE (CONFIG_ID, VERSION_LABEL);
...
CREATE TABLE DOCUMENT_PREVIEW (
      ID NUMBER(22,0) GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      DOCUMENT_VERSION_ID NUMBER(22,0) NOT NULL,
      DOCUMENT_REF VARCHAR2(400 BYTE) NOT NULL,
      VERSION_LABEL VARCHAR2(100 BYTE) NOT NULL,
      CONTENT BLOB,
      STATUS VARCHAR2(30 BYTE) DEFAULT 'IN_PROGRESS',
      AUDIT_C_DATE TIMESTAMP(6) DEFAULT SYSTIMESTAMP NOT NULL,
      AUDIT_M_DATE TIMESTAMP(6),
      CONSTRAINT DOCUMENT_PREVIEW_FK FOREIGN KEY (DOCUMENT_VERSION_ID) REFERENCES DOCUMENT_VERSION(ID) ON DELETE CASCADE,
      CONSTRAINT DOCUMENT_PREVIEW_REF_VER_UQ UNIQUE (DOCUMENT_REF, VERSION_LABEL)
);

CREATE INDEX DOC_PREVIEW_DOCV_IDX ON DOCUMENT_PREVIEW(DOCUMENT_VERSION_ID);
CREATE INDEX DOC_PREVIEW_VERSION_IDX ON DOCUMENT_PREVIEW(VERSION_LABEL);
