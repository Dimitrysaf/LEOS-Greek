...
(	 ID INT NOT NULL AUTO_INCREMENT,
      CMIS_ID VARCHAR(100) NOT NULL COMMENT "Reference to CMIS in case of migration",
      NAME VARCHAR(100) NOT NULL COMMENT "Name of the repository",
      DESCRIPTION VARCHAR(100) NOT NULL COMMENT "Description for this repository",
      ALL_VERSIONS_SEARCHABLE DECIMAL(1,0) COMMENT "Search for all versions",
      AUDIT_C_BY VARCHAR(30),
      AUDIT_C_DATE TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      ...
      CONSTRAINT REPOSITORY_UQ UNIQUE (NAME)
) COMMENT "Definition of all repositories for LEOS" TABLESPACE leos_repository;

CREATE TABLE PERMISSION
( ID INT NOT NULL AUTO_INCREMENT,
  REPOSITORY_ID INT NOT NULL COMMENT "ID for the repository this permission belongs to",
  ...
  CONSTRAINT PERMISSION_FK FOREIGN KEY (REPOSITORY_ID) REFERENCES REPOSITORY (ID)
...
( ID INT NOT NULL AUTO_INCREMENT,
  REPOSITORY_ID DECIMAL NOT NULL COMMENT "Repository logging data for",
...
( ID INT NOT NULL AUTO_INCREMENT,
  OBJECT_ID DECIMAL NOT NULL COMMENT "Internal column CMIS for migration",
...
( ID INT NOT NULL AUTO_INCREMENT,
  ...
  EXPORT_STATUS VARCHAR(30) COMMENT "Export status",
  EXPORT_DATE TIMESTAMP COMMENT "Export date",
  AUDIT_C_BY VARCHAR(30) NOT NULL COMMENT "Audit column holding the user that created this record",
  AUDIT_C_DATE TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL COMMENT "Audit column holding the date at which this record was created",
  AUDIT_LAST_M_BY VARCHAR(30) COMMENT "Audit column holding the user of the last update on this record",
  AUDIT_LAST_M_DATE TIMESTAMP COMMENT "Audit column holding the date of the last update on this record",
...
( ID INT NOT NULL AUTO_INCREMENT,
  ...
  OBJECT_ID DECIMAL NOT NULL COMMENT "Internal CMIS id for migration",
  CATEGORY_ID INT NOT NULL COMMENT "Category ID for the configuration",
  ORIGINAL_ID DECIMAL COMMENT "Original CMIS id or migration",
  REPOSITORY_ID DECIMAL COMMENT "Repository in which this configuration file is stored",
  AUDIT_C_BY VARCHAR(30) NOT NULL COMMENT "Audit column holding the user that created this record",
  AUDIT_C_DATE TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL COMMENT "Audit column holding the date at which this record was created",
  AUDIT_LAST_M_BY VARCHAR(30) COMMENT "Audit column holding the user of the last update on this record",
  AUDIT_LAST_M_DATE TIMESTAMP COMMENT "Audit column holding the date of the last update on this record",
  ...
  CONSTRAINT CONFIG_UQ UNIQUE (NAME, REPOSITORY_ID)
...
(
    ...
    CONSTRAINT COLLABORATORS_UQ UNIQUE (COLLABORATOR_NAME, ROLE_ID)
...
SELECT CONCAT(doc.id,docver.id,docxml.id,doccat.id) unique_id
...
SELECT milv.milestone_id,millis.id unique_id, milv.package_id, milv.document_id
...
SELECT CONCAT(conf.id,ver.id,con.id,cat.id) unique_id, conf.ID, CONF.NAME, CONF.OBJECT_ID,
...
CREATE OR REPLACE VIEW DOCUMENT_PROPERTIES_V as
SELECT concat(doc.id,cat.id,docver.id,pro.id,proval.id) unique_id, doc.id,doc.object_id,doc.package_id,doc.category_id,cat.category_code,cat.category_desc,doc.name
