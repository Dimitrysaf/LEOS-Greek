...
public class CmisRepositoryContext implements RepositoryContext {
    ...
    private static final RepositoryPropertiesMapper repositoryPropertiesMapper = new CmisProperties();
    
    private static final Map<String, String> oldVersions = new HashMap<>();

    public CmisRepositoryContext(CmisRepository cmisRepository) {
        ...
        if (allVersionsIterator.hasPrevious()) {
            do {
                ...
                if (StringUtils.isEmpty(versionLabel)) {
                    ...
                    } else if ((previousVersion.getProperty(repositoryPropertiesMapper.getId(RepositoryProperties.MILESTONE_COMMENTS)) != null) &&
                            (!previousVersion.getProperty(repositoryPropertiesMapper.getId(RepositoryProperties.MILESTONE_COMMENTS)).getValues().isEmpty())) {
