...
@Component
public class NumberProcessorHierarchicalBasedDefault extends NumberProcessorAbstract implements NumberProcessorDepthBased {

	private static final Logger LOG = LoggerFactory.getLogger(NumberProcessorHierarchicalBasedDefault.class);

    public NumberProcessorHierarchicalBasedDefault(MessageHelper messageHelper, NumberProcessorHandler numberProcessorHandler, SecurityContext securityContext, TrackChangesContext trackChangesContext) {
        super(messageHelper, numberProcessorHandler, securityContext, trackChangesContext);
    }

    @Override
    public boolean canRenumber(Node node) {
        return RECITALS.equals(node.getNodeName());
    }
