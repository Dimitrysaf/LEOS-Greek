    ...
    var ConfigNormalizerStamp = STAMPIT().methods(
            {
                ...
                __normalizeRawConfigForAknToHtmlForToSide: function __normalizeRawConfigForAknToHtmlForToSide(normalizedConfigTo, rawConfig) {
                    ...
                    //LEOS-2259: adding validation on html attributes, should be lower case
                    if (!!configPathAttributeResolverForTo.attributeName && configPathAttributeResolverForTo.attributeName != configPathAttributeResolverForTo.attributeName.toLowerCase()) {
                        throw "Invalid attribute name  (HTML Attribute Name should be all lower case): '"+configPathAttributeResolverForTo.attributeName+"'"+"for config: "+ JSON.stringify(this._.rawConfig).replace(/"/g,'');
                    }
                ...
                _normalizeSingleAttrTo: function(attr) {
                    ...
                    //LEOS-2259: adding validation on html attributes, should be lower case
                    if (!!htmlNorm.name && htmlNorm.name != htmlNorm.name.toLowerCase()) {
                        throw "Invalid attribute name  (HTML Attribute Name should be all lower case): '"+htmlNorm.name+"'"+"for config: "+ JSON.stringify(this._.rawConfig).replace(/"/g,'');
                    }
