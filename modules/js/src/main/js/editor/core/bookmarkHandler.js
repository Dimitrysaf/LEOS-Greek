...
define(function bookmarkHandlerModule(require) {
  ...
  function _createBookmark(event) {
    ...
    var baseUri = CKEDITOR.basePath.substring(
      0,
      CKEDITOR.basePath.indexOf("lib/ckeditor_")
    );
    if (!baseUri.startsWith(window.location.origin)) {
      baseUri = baseUri.replace(/^(https?:)?\/\/[^/]+/, window.location.origin);
    }
    $bookmark.load(
      baseUri +
        "js/editor/core/templates/bookmarkTemplate_" +
        language +
        ".html"
    );
    ...
    $bookmark.on(
      "click.navigation",
      _navigateToEditor.bind(undefined, editor, element)
    );
