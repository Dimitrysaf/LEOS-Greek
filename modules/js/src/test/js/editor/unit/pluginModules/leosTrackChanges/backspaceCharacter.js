...
define(function deleteCharacter(require) {
        ...
        it("when backspacing single character (should create 3 nodes, where the 2nd is the trackChange element)", function (done) {
            ...
            // editor.on('contentDom', function (evt) {
                if(!processing) {
                    ...
                    createSelectionSplitting(editor, selectedElement, 3)
    ...
    function createSelectionSplitting(editor, selectedElement, splitIndex) {
        var range = editor.getSelection().getRanges()[0]
        var targetPosition = range.getNextNode() // content of <p> "hello world"
        var actualContent = targetPosition?.$?.data
        var arr = split_at_index(actualContent, splitIndex)
        var firstChild = new CKEDITOR.dom.text(arr[0]);
        var secondChild = new CKEDITOR.dom.text(arr[1]);

        var actualNode = range.root.getChildren().getItem(0) //p

        actualNode.getChildren().getItem(0).remove()
        firstChild.appendTo(actualNode)
        secondChild.appendTo(actualNode)

        range.moveToPosition(secondChild,  CKEDITOR.POSITION_AFTER_START)
        range.select();
        console.log("Selection: Nr. R")
    }
