...
import { ContributionAnnotateConnector } from './contribution-annotate-connector';
...
export class ContributionAnnotateManager {
  private connector?: ContributionAnnotateConnector;
  ...
  constructor(
    ...
    const connector$ = this.createConnectorState().pipe(
      ...
      map(
        (state) =>
          new ContributionAnnotateConnector(
            state,
            {
              permissions,
